...
1. Tree clustering  -- hierarchical clustering on a correlation-distance
   matrix, wrapped by ``tree.cluster_tree.ClusterTree`` (which itself
...
def allocate(
    ...
    See ``tests/unit/test_hrp_nan_behavior.py`` for the experiments that
